...
Array.from(document.getElementsByClassName('songItemPlay')).forEach((element) => {
    element.addEventListener('click', (e) => {
        ...
        if (audioElement.paused || audioElement.currentTime <= 0) {
            ...
            editsong.classList.remove('fa-pause-circle');
            editsong.classList.add('fa-play-circle');
        }
    });
});
masterPlay.addEventListener('click', () => {
    if (audioElement.paused || audioElement.currentTime <= 0) {
        // let iid = document.getElementsByClassName("songItemPlay").target;
        audioElement.play();
        masterPlay.classList.remove('fa-play-circle');
        masterPlay.classList.add('fa-pause-circle');
        editsong.classList.remove('fa-play-circle');
        editsong.classList.add('fa-pause-circle');
        gif.style.opacity = 1;
    }
    else {
        audioElement.pause();
...
document.getElementById("previous").addEventListener('click', () => {
    ...
    let songg = document.getElementById(songIndex+1).target
    console.log(songg.class)
    if (songIndex <= 0) {
        songIndex = 0;
    }
    else {
        songIndex -= 1;
        songg.classList.remove('fa-play-circle');
        songg.classList.add('fa-pause-circle');
    }
    audioElement.src = `songs/${songIndex}.mp3`;
    mastersongname.innerText = songs[songIndex].songname;
